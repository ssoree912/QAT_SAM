...
def get_args_parser():
    ...
    parser.add_argument('--wandb', action='store_true',
                        help='log train loss / test acc to Weights & Biases')
    parser.add_argument('--wandb-project', default='aoq-vit-qat', type=str)
...
def main(args):
    ...
    # wandb: rank-0 only, and not in --eval mode (that path returns before the train loop).
    use_wandb = args.wandb and utils.is_main_process() and not args.eval
    if use_wandb:
        import wandb
        wandb.init(
    ...
    if args.finetune:
        ...
        # Role-1 proof (see plan §verify 4): FP weights MUST be loaded; only quant params
        # (alpha/zero_point/init_state/signed), the extra norm_q/norm_k, and head* may be missing.
        print(f"[finetune] init from {args.finetune}")
        print(f"[finetune] missing ({len(load_result.missing_keys)}): {load_result.missing_keys[:10]}")
        print(f"[finetune] unexpected ({len(load_result.unexpected_keys)}): {load_result.unexpected_keys[:10]}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if use_wandb:
            wandb.log({**log_stats, 'max_accuracy': max_accuracy}, step=epoch)
    ...
    if use_wandb:
        wandb.summary['max_accuracy'] = max_accuracy
        wandb.finish()
