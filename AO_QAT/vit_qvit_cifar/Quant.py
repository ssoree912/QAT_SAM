...
def _qsam_perturbation(weight, g_prev, delta_step, ratio, rho):
    ...
    g_detached = g_prev.detach()
    delta_detached = delta_step.detach().to(weight.dtype)

    if weight.dim() > 1 and delta_detached.numel() == weight.shape[0]:
        g_by_group = g_detached.abs().reshape(weight.shape[0], -1)
        group_size = g_by_group.shape[1]
        k = min(group_size, max(1, int(round(ratio * group_size))))
        _, idx = torch.topk(g_by_group, k, dim=1)
        mask_by_group = torch.zeros_like(g_by_group, dtype=weight.dtype)
        mask_by_group.scatter_(1, idx, 1.0)
        mask = mask_by_group.view_as(weight)
    else:
        n = weight.numel()
        k = min(n, max(1, int(round(ratio * n))))
        _, idx = torch.topk(g_detached.abs().reshape(-1), k)
        mask = torch.zeros(n, device=weight.device, dtype=weight.dtype)
        mask[idx] = 1.0
        mask = mask.view_as(weight)

    return (rho * delta_detached * torch.sign(g_detached) * mask).detach()
...
class Conv2dQ(_Conv2dQ):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        # qSAM (disabled until enable_qsam registers g_prev and flips use_qsam)
        self.use_qsam = False
        self.qsam_ratio = 1e-3
        self.qsam_rho = 1.0
        self.qsam_on = True

    def save_grad_for_qsam(self):
        if getattr(self, 'use_qsam', False) and self.weight.grad is not None:
    ...
    def forward(self, x):
        ...
        if getattr(self, 'use_qsam', False) and self.training and self.qsam_on:
            d = _qsam_perturbation(self.weight, self.g_prev, alpha,
                                   self.qsam_ratio, self.qsam_rho)
            if d is not None:
                w_q = w_q + d
